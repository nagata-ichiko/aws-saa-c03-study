...
| 2026-03-25 ミニ試験 | 11 | 6 | 54.5% | ❌ 未達（採点ミス修正済み） |
| 2026-03-27 フル試験（途中終了） | 48 | 21 | 43.8% | ❌ 未達 |
...
| SSL証明書の管理サービス（IAM vs ACM） | Q371 | SSL証明書の管理はIAM（旧来）またはACM。Route 53はDNSサービスであり証明書管理はできない ❌ ミニ試験不正解(2026-03-25) |
| VPCウィザードのテンプレート種類 | Q519 | VPCウィザード4種: ①パブリックのみ ②パブリック+プライベート ③パブリック+プライベート+VPN ④プライベートのみ+VPN。「パブリックのみ+VPN」は存在しない ❌ ミニ試験不正解(2026-03-25) |
| CloudHSM セキュリティグループ要件 | Q207 | CloudHSM初期設定にはSSH(22)またはRDP(3389)ポートを開ける必要がある ❌ ミニ試験不正解(2026-03-25) |
...
| ELBゾーン追加 | Q552 | 既存ELBにAZをリアルタイムで追加可能。ELBを停止する必要はない ✅ 解説確認済み(2026-03-25) ✅ ミニ試験正解(2026-03-25) |
...
| NLB接続ドレイニング | Q1730 | TCP/UDP→NLB。メンテナンス時の接続維持→接続ドレイニング有効化。ALBはHTTP/HTTPSのみ ❌ ミニ試験不正解(2026-03-25) |
| Auto Scalingクールダウン計算 | Q261 | クールダウンは最後のインスタンス起動から計算。2番目が4分後→4+7=11分（最初から） ✅ ミニ試験正解(2026-03-25) |
...
| DynamoDBオンデマンドキャパシティ | Q1760 | 一時的スロットリング+最小運用負荷→オンデマンドキャパシティモード。DynamoDB Streamsは変更検知用で書き込み性能向上に無関係 ❌ ミニ試験不正解(2026-03-25) |
| Amazon VPCの役割 | Q589 | 従来データセンターに近い仮想ネットワーク→Amazon VPC。Amazon Service BusはAzureのサービスでAWSには存在しない ✅ ミニ試験正解(2026-03-25) |
...
| AWS Budgets + Compute Optimizer | Q1771 | リアルタイムコストアラーム→AWS Budgets。リソース最適化提案→Compute Optimizer。Cost Explorerの週次レポートは手動作業で運用負荷高 ❌ ミニ試験不正解(2026-03-25) |
...
| 2026-03-27 | フル模擬試験（48問で途中終了）の結果を記録。正答率 43.8%（21/48）。D1: 33.3%, D2: 40.0%, D3: 63.6%, D4: 42.9%。SNS/SQS混同・EC2購入オプション・Auto Scaling通知設定・RDSリードレプリカ vs Multi-AZ の弱点が継続。Global Accelerator vs Route 53 の使い分けも要復習 |
