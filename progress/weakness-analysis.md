...
| ELBゾーン追加 | Q552 | 既存ELBにAZをリアルタイムで追加可能。ELBを停止する必要はない ✅ 解説確認済み(2026-03-25) ❌ ミニ試験不正解(2026-03-25) 再強化必要 |
...
| Auto Scalingクールダウン計算 | Q261 | クールダウンは最後のインスタンス起動から計算。2番目が4分後→4+7=11分（最初から） ❌ ミニ試験不正解(2026-03-25) 再強化必要 |
...
| Amazon VPCの役割 | Q589 | 従来データセンターに近い仮想ネットワーク→Amazon VPC。Amazon Service BusはAzureのサービスでAWSには存在しない ❌ ミニ試験不正解(2026-03-25) |
