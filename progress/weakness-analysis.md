...
| 🔴 Route 53 ルーティングポリシーの組み合わせ | Q31, Q340 | レイテンシー最適化 + フェイルオーバー = レイテンシールーティング + ヘルスチェックの組み合わせ。**再度間違えた（2回目）**: 「同等の体験」= 加重ルーティング（均等分散）が正解。ジオロケーション = 地域ごとに異なるルーティング（ローカライズ・地域制限）であり「同等の体験」ではない。**【重点弱点】弱点復習でも再度間違い — 加重 vs ジオロケーションの判断基準を根本から再学習すること** |
...
| 🔴 Direct Connect / VPN 移行手順と BGP 設定 | Q358 | DX移行の正しい順序: ①DXルーター設定→②VPCルートテーブル更新→③VPNのBGP優先度調整→④確認。BGP優先度の設定を忘れずに。VPNはバックアップとして残しダウンタイムなしで移行する。**弱点復習でも再度間違い — DX移行手順の各ステップを順序通りに暗記すること** |
...
| 🔴 EBS Provisioned IOPS の CloudWatch メトリクス | Q105, Q463 | Provisioned IOPS ボリュームの CloudWatch メトリクス送信間隔は1分。汎用SSD等は5分間隔。**再度間違えた（2回目）**: IOPS超過の確認 = VolumeQueueLength（平均キュー長）が正解。レイテンシ（ストレージサブシステムの応答）はIOPS超過の「結果」であり「原因確認」には使えない。**弱点復習でも再度間違い — VolumeQueueLength の意味を確実に定着させること** |
| 🔴 AWS Storage Gateway の機能理解 | Q56, Q342 | Storage Gateway の各タイプ（File/Volume/Tape）の用途と機能を整理する必要あり。**再度間違えた（2回目）**: Gateway-Cached = 本体S3 + キャッシュのみローカル、Gateway-Stored = 全データローカル + S3にバックアップ。DはAの説明が逆の「ひっかけ選択肢」に引っかかった。**【重点弱点】弱点復習でも再度間違い — Cached vs Stored の定義を確実に暗記すること** |
...
| トピック | 間違えた問題 | 具体的な誤解・ギャップ |
| :--- | :--- | :--- |
| 🔴 Spot インスタンスの活用 | Q24, Q331, Q557 | 分散処理・バッチ処理・障害耐性のある分散処理には Spot インスタンスが最もコスト効率が高い。**再度出題**: 障害耐性ありの分散処理コスト最適化 = Spot Instances。**3回目（Q557）**: 短時間（10分）・一時的（1〜2回）・中断許容のバッチ処理に RI を選択してしまった。RI は1〜3年の長期コミットが必要で一時処理には不適切。**弱点復習でも再度間違い（4回目）— 【最重点弱点】EC2購入オプション判断フロー: 中断許容 → Spot、長期安定 → RI、短期確実 → オンデマンド。この判断を条件反射レベルで定着させること** |
...
| S3 ストレージクラス（Glacier Deep Archive）| Q687 | 🟡 **克服傾向あり**: 弱点復習で正解。S3 ストレージクラスの取得時間・コスト比較の理解が定着してきている |
| SQS デカップリングパターン | Q361 | 🟡 **克服傾向あり**: 弱点復習で正解。SQS によるマイクロサービス間のデカップリング設計を正しく選択できた |
...
### 🚨 最重点弱点（複数回間違い・弱点復習でも不正解）

| トピック | 間違い回数 | ドメイン |
| :--- | ---: | :--- |
| EC2 購入オプション Spot vs RI（Q24, Q331, Q557） | 4回 | Domain 4 |
| S3 暗号化方式の使い分け（ミニQ7, Q146） | 3回 | Domain 1 |
| Route 53 ルーティングポリシー 加重 vs ジオロケーション（Q31, Q340） | 2回 | Domain 2 |
| Storage Gateway Cached vs Stored（Q56, Q342） | 2回 | Domain 3 |
| EBS CloudWatch VolumeQueueLength（Q105, Q463） | 2回 | Domain 3 |
| Direct Connect/VPN 移行手順・BGP設定（Q358） | 2回 | Domain 3 |
| マルチリージョン低レイテンシー DynamoDB vs ElastiCache（Q158） | 2回 | Domain 3 |
...
2. **Domain 2（22.2%）**: 🔴 Route 53 加重 vs ジオロケーション（重点）、ELB種類別使い分け（ALB/NLB）、Kinesis vs SQS vs SNS、VPN CloudHub、グローバルサービス vs リージョナルサービス
3. **Domain 4（28.6%）**: 🔴 EC2購入オプション判断フロー（最重点）、S3ストレージクラス全種類（🟡 Glacier Deep Archive は克服傾向）、EMR+Redshiftコスト最適化
4. **Domain 3（33.3%）**: 🔴 Storage Gateway Cached vs Stored（重点）、🔴 EBS VolumeQueueLength（重点）、🔴 DX/VPN移行手順（重点）、EC2+EBSパフォーマンスチューニング
...
| 2026-03-23 | 弱点復習（7問）正答率 28.6%（2/7）。正解: Q687(S3ストレージクラス)→克服傾向、Q361(SQSデカップリング)→克服傾向。不正解: Q340(Route 53: 2回目)、Q342(Storage Gateway: 2回目)、Q557(Spot: 4回目)、Q463(EBS VolumeQueueLength: 2回目)、Q358(DX/VPN移行: 2回目)。重点弱点テーブルを新設し繰り返し間違いを可視化 |
