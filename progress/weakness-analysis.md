...
| Route 53 ルーティングポリシーの組み合わせ | Q31, Q340 | レイテンシー最適化 + フェイルオーバー = レイテンシールーティング + ヘルスチェックの組み合わせ。**再度間違えた**: 「同等の体験」= 加重ルーティング（均等分散）が正解。ジオロケーション = 地域ごとに異なるルーティング（ローカライズ・地域制限）であり「同等の体験」ではない |
...
| トピック | 間違えた問題 | 具体的な誤解・ギャップ |
| :--- | :--- | :--- |
| Spot インスタンスの活用 | Q24, Q331, Q557 | 分散処理・バッチ処理・障害耐性のある分散処理には Spot インスタンスが最もコスト効率が高い。**再度出題**: 障害耐性ありの分散処理コスト最適化 = Spot Instances。**3回目（Q557）**: 短時間（10分）・一時的（1〜2回）・中断許容のバッチ処理に RI を選択してしまった。RI は1〜3年の長期コミットが必要で一時処理には不適切。Spot の典型ユースケースを見逃した |
...
| 2026-03-23 | Q557(EC2購入オプション: Spot vs RI) 不正解。Spotインスタンスの弱点が継続（3回目）。短時間・一時的・中断許容バッチ処理にRIを選択。購入オプション判断フローの定着が必要 |
